...
import { localStorageMock } from '../utils/MockedLocalStorage'
...
describe('Integration - Client login/logout & password auth provider', () => {
    ...
    it(`When a user doesn't exist, it can still login`, async () => {
        ...
        // Login
        const { ethAddress, timestamp, authChain } = getLoginData()
        const client = await SocialClient.loginToServer(synapseContainer.getAddress(), ethAddress, timestamp, authChain, localStorageMock())
    ...
    it(`When a user already exists, it can login many times`, async () => {
        ...
        const client = await SocialClient.loginToServer(synapseContainer.getAddress(), identity.address, timestamp, authChain, localStorageMock())
        ...
        const client2 = await SocialClient.loginToServer(synapseContainer.getAddress(), identity.address, timestamp2, authChain2, localStorageMock())
